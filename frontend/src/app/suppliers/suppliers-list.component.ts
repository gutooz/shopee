...
@Component({
      ...
      <div class="page-container">
        ...
        @if (showForm()) {
          <div class="form-panel">
            <h3>{{ editingId() ? 'Editar' : 'Novo' }} Fornecedor</h3>
            <form [formGroup]="form" (ngSubmit)="save()">
              <div class="form-grid">
                ...
                <mat-form-field appearance="outline">
                  ...
                  <mat-icon matSuffix style="color: #25d366; font-size: 18px">chat</mat-icon>
                </mat-form-field>
            ...
            <table mat-table [dataSource]="suppliers()">
              <ng-container matColumnDef="name">
                ...
                <td mat-cell *matCellDef="let s">
                  <div class="sup-cell">
                    <div class="sup-icon">{{ s.name[0].toUpperCase() }}</div>
                    {{ s.name }}
                  </div>
                </td>
              </ng-container>
              <ng-container matColumnDef="email">
                ...
                <td mat-cell *matCellDef="let s">
                  <span class="cell-dim">{{ s.email || '—' }}</span>
                </td>
              </ng-container>
              <ng-container matColumnDef="phone">
                <th mat-header-cell *matHeaderCellDef>Telefone</th>
                <td mat-cell *matCellDef="let s">
                  <span class="cell-dim">{{ s.phone ?? '—' }}</span>
                </td>
              </ng-container>
              ...
              <ng-container matColumnDef="actions">
                ...
                <td mat-cell *matCellDef="let s">
                  <button mat-icon-button (click)="edit(s)" title="Editar">
                    <mat-icon>edit</mat-icon>
                  </button>
                  <button mat-icon-button color="warn" (click)="delete(s.id)" title="Excluir">
  ...
  styles: [`
    ...
    .sup-icon {
      ...
      color: var(--fg-2); font-size: 0.7rem; font-weight: 700;
...
export class SuppliersListComponent implements OnInit {
  ...
  load(): void {
    ...
    this.api.getSuppliers(this.page, this.pageSize).subscribe({
      next: data => { this.suppliers.set(data.items); this.total.set(data.total); this.loading.set(false); },
      error: () => this.loading.set(false),
    });
  }

  openForm(): void { this.editingId.set(null); this.form.reset(); this.showForm.set(true); }
  edit(s: Supplier): void { this.editingId.set(s.id); this.form.patchValue(s); this.showForm.set(true); }
  ...
  save(): void {
    ...
    const obs = id ? this.api.updateSupplier(id, this.form.value) : this.api.createSupplier(this.form.value);
    obs.subscribe({
      next: () => {
        this.toast.success(id ? 'Fornecedor atualizado!' : 'Fornecedor criado!');
        this.cancelForm(); this.load(); this.saving.set(false);
      },
      error: err => { this.toast.error(err.error?.detail ?? 'Erro ao salvar'); this.saving.set(false); },
    });
  }
